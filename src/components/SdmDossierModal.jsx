...
import SdmDocumentPreviewModal from "./SdmDocumentPreviewModal";
  ...
  const handleNativeFileChange = async (e) => {
    ...
    reader.onload = (event) => {
      ...
      const newDoc = {
        id: `DOC-${Date.now()}`,
        nama: displayName,
        kategori: "Dokumen Kepegawaian",
        tipe: extension,
        ukuran: formattedSize,
        tanggalUpload: new Date().toISOString().split("T")[0],
        status: "Terverifikasi",
        fileUrl: base64Url || URL.createObjectURL(file),
    ...
    reader.onerror = () => {
      ...
      if (fileInputRef.current) fileInputRef.current.value = "";
      const newCompleteness = Math.min(100, Math.round((updatedDocs.length / 5) * 100));
      onSaveDossier?.({ ...dossier, persentaseLengkap: newCompleteness, dokumen: updatedDocs });
    };
  ...
  return (
    <>
          ...
          <div className="p-4 overflow-y-auto" style={{ flex: 1 }}>
            ...
            >
              ...
              <div className="row g-2 align-items-center">
                <div className="col-md-7">
                  <input
                    ...
                    placeholder="Keterangan Dokumen (contoh: Ijazah SD, SK Pangkat 2026)..."
                ...
                <div className="col-md-5">
                  <button
                    ...
                    className="btn btn-success btn-sm w-100 fw-semibold d-flex align-items-center justify-content-center gap-2 shadow-sm hover-lift"
              ...
              <small className="text-muted d-block mt-2" style={{ fontSize: "0.72rem" }}>
                Mendukung format: <strong>PDF, DOCX, JPG, PNG, WEBP</strong> (Maks. 25 MB per dokumen)
              </small>
            </div>
                      ...
                      <tr key={doc.id || idx} style={{ borderBottomColor: borderColor }}>
                        <td className="px-3 py-2 fw-medium">
                          ...
                          >
                            ...
                            <div>
                              <span className="d-block fw-semibold">{doc.nama}</span>
                              {doc.realFileName && (
                                <small className="text-muted" style={{ fontSize: "0.68rem" }}>
                                  File: {doc.realFileName}
                                </small>
                              )}
                            </div>
                          </div>
                        ...
                        <td className="text-end px-3 py-2">
                          <div className="btn-group btn-group-sm">
                            <button
                              type="button"
                              className="btn btn-outline-success p-1 px-2"
      ...
      <SdmDocumentPreviewModal
        ...
        employeeName={dossier.nama}
        employeeNip={dossier.employeeId}
        employeeUnit="RSJ Tampan"
        darkMode={darkMode}
      />
    </>
  );
